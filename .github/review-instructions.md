...
You MUST read `review/head/.specify/memory/constitution.md` in full every round. It is short. When the diff edits it, the edit is itself under review, and it MUST carry what Governance asks: a note of what changed and why, and the right kind of version bump. When the diff also adds or edits a spec whose Clarifications record the owner approving that amendment, the amended text binds the rest of the diff, since the owner has ruled on it and a finding against the old text would only repeat the ruling. Otherwise the text before the edit binds this review, so a change cannot license itself. Reconstruct that text from the head copy and `review/pr.diff`: in each hunk that touches the constitution, drop the `+` lines and restore the `-` lines. The default-branch copy is not an option, because it sits outside `review/`, where the base prompt forbids hidden directories as context. The constitution's seven Core Principles outrank every other document that binds the code under review. A **verified** change that departs from one of them is a finding even if another document permits it, and the principle SHOULD be named. Suspecting a departure you could not trace is a question, as it would be anywhere else. The ones most often at stake in a diff:
