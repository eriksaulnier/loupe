...
| Record | Version | Note |
| :--- | :--- | :--- |
| draft, hand-back, target, previous, comments | `schema` 1 | Strict. Unchanged until the next field change |
| attempt, receipt | `schema` 1 | `author`, `edited` and `envelope.editReviewId` came without a bump. They stay at 1, because a bump now helps only binaries that ship the new reader |
| findings record | `v=1` | Lenient data |
