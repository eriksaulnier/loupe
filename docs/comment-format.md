...
- **An unattended sticky round MAY carry a note under its footer** (`loupe publish --unattended --sticky --note`, `specs/032-sticky-round-note`). After the footer comes a blank line, `<!-- loupe-note -->`, a blank line, the note, a blank line and `<!-- loupe-note-end -->`, in the footer's block and before the divider that opens the earlier rounds. The note passes the allowlist at the summary's depth. It belongs to the newest round only: read-back drops it, delimiters included, before it collapses the round, so a collapsed round never carries a note and is the same whether or not the round had one. A body whose note delimiters are not one pair after a blank line, on the round on top, is refused with `sticky`. The note is not a finding and is not covered by the digest. A loupe released before the note cannot continue a series whose newest round carries one: it refuses with `sticky`, whose fix, publishing once without `--sticky`, still works.
