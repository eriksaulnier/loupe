...
- **A collapsed round is carried as it is, even when a line of its quote lost its `>` on GitHub.** That line renders outside the quote, and nothing else changes. Only a hidden marker could tell that edit from an earlier layout's prose that ends in a quote and a line shaped like a footer, so loupe does not guess.
