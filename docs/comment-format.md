...
- **The second is the findings record** (`specs/027-previous-from-github`). It lets the next round's capture read this round back from GitHub, so `loupe show --previous` works where no local receipt from the same publisher exists, such as a CI run that starts from an empty data root. It sits between the reconciliation marker and `loupe-meta`, on every review, attended or unattended, sticky or not. A body carries at most one.
  ...
  - **Version 2 carries the round's assessments** (`specs/031-open-findings`). A round that ran `loupe assess` writes `v=2`, whose `<data>` is the same encoding of an object: `{"findings": [...], "assessments": [...]}`, where `findings` is version 1's array and each assessment is `{"ref", "status", "finding"}`. `status` is `open` or `addressed`. `finding` is the earlier finding as the round assessed it: version 1's fields plus `"filedIn": {"round", "reviewUrl", "commit"}`, the round that first filed it, with `commit` absent when it could not be read back. `ref` is the name the assessing round used, and a later round names it afresh. A round that assessed nothing writes version 1, byte for byte as before, so a loupe that reads only version 1 still reads it. The next round lists the findings marked `open`, then this round's own, as every finding still open. Assessments add no visible text.
  ...
  - A reader that finds no record, an omission, more than one record, a `v=` other than `1` or `2`, a checksum mismatch, data that does not decode, data not in its version's shape, or an assessment without a known status or its finding's id and title reports that there is no previous round and why. It never falls back to an older review.
  - When `v=` MUST change is set by the [schema versioning rule](versioning.md).
  - **Terminal surfaces show a note, not the data.** The publish confirmation shows the body as raw Markdown, and it replaces the record there with `<!-- loupe-findings: a copy of the N findings above -->` (`the 1 finding above`, `no findings`), as it shows each pill as its word. A version 2 record's note goes on to name the assessments an attended round publishes under the human's name: `, plus N earlier findings still open`, `, plus N earlier findings marked addressed`, or `, plus N earlier findings, O still open and A addressed`, with `1 earlier finding` for one. The payload view shows the exact bytes.
...
````markdown
<!-- loupe-round v=1 n=2 commit=<sha> blocking=0 issues=0 suggestions=0 questions=1 other=0 prose=1 note=0 sha256=<sha256> -->
...
<!-- loupe-round v=1 n=1 commit=<sha> sha256=<sha256> -->
...
- **An unattended sticky round MAY carry a note under its footer** (`loupe publish --unattended --sticky --note`, `specs/032-sticky-round-note`, amended by `specs/033-round-anchors`). After the footer comes a blank line, then the note written as one quote (each line prefixed `> `, a blank line as `>`), in the footer's block and before the divider that opens the earlier rounds. The top anchor's `note=` counts its lines, quote markers included. The note passes the allowlist at the summary's depth. It belongs to the newest round only: read-back drops those lines before it collapses the round, so a collapsed round never carries a note and is the same whether or not the round had one. The note is not a finding and is not covered by the digest. A loupe released before the note cannot continue a series whose newest round carries one: it refuses with `sticky`, whose fix, publishing once without `--sticky`, still works.
- **Each round opens on an anchor** (`specs/033-round-anchors`): one hidden line `<!-- loupe-round v=1 n=N commit=SHA … sha256=HEX -->`, alone and outside any fence, followed by a blank line. The anchor of the newest round is line 1 of the body. Each collapsed round's anchor sits right before its `<details>`. A review without `--sticky` carries none.
  - Every anchor carries `v`, `n`, the round's place in the sticky review, and `commit`, the round's full commit when loupe knew it, and ends with `sha256`. The newest round's anchor also carries `blocking`, `issues`, `suggestions`, `questions` and `other`, the counts its chips row shows, where `issues` and the three after it count non-blocking findings only, unlike `loupe-meta`'s census. It then carries `prose`, the lines of opening prose, and `note`, the lines of the note, 0 for none. New keys MAY be added before `sha256`. Existing keys MUST keep their meaning. Every value is a decimal integer or lowercase hex, so no value can close the comment.
  - `sha256` is the SHA-256 hex of the anchor's text between `<!-- loupe-round ` and ` sha256=`, a newline, then the round's text: the lines after the anchor up to the next boundary, CRLF and lone CR read as LF, with blank lines trimmed at both ends. The newest round runs to the divider before `<!-- loupe-earlier -->`, or to its reconciliation marker when there is no earlier round, and its text ends with a newline and that marker. A collapsed round runs to the next anchor, or to the newest round's reconciliation marker.
  - The newest round's opening prose and note are written with blank lines trimmed at both ends, so the counts hold. GitHub renders the same either way.
  - **A round whose checksum does not match was edited on GitHub, and is carried as found.** A collapsed round keeps every byte of its block. The newest round is collapsed with its whole text quoted, chips row, dividers and note included, and so is a newest round with text above its anchor, with the lines above the anchor quoted first. It sits under the summary line its anchor's counts and commit give, and nothing else in it is removed or rewritten. Whitespace-only lines above the top anchor are not content: read-back drops them and does not name the round edited. Either way, the anchor is written again with its other fields as found and a checksum over what was carried, so only the publication that finds the edit names it. The attended confirmation names each such round before the body, and an unattended round names it on stderr.
  - The confirmation shows each anchor as `<!-- loupe-round N -->`. The payload view shows the exact bytes.
  - GitHub renders a body that opens on an anchor with nothing above the chips row (`docs/github-facts.md`).
...
- **A body in the v0.11.0 layout** (the newest round's footer after the earlier rounds, and collapsed rounds without a footer) still reads back, and the next round writes this layout. The round it showed keeps its footer. The rounds v0.11.0 collapsed have no footer to recover and are carried as they are. One such body is refused with `sticky`: the shown round's prose ends in a divider and a line shaped like a footer, and either every earlier round was dropped or the newest collapsed round's last line is also shaped like a footer, since nothing then tells it from a footer added on GitHub.
- **A body with no anchor** was written before `specs/033-round-anchors` and is read by the rules this section gives for earlier bodies and for `<!-- loupe-round -->`, which are frozen: they MUST NOT gain a rule, and none is removed. The next round anchors every round it carries, so a series reads a body without anchors at most once. That includes a body in the v0.13.0 layout, and one whose newest round carries a note between `<!-- loupe-note -->` and `<!-- loupe-note-end -->`, as `specs/032-sticky-round-note` first wrote it. That body's note delimiters MUST be one pair after a blank line, right after the footer of the round on top, or it is refused with `sticky`. A body that carries both an anchor and `<!-- loupe-round -->` is refused with `sticky`.
...
- **Hidden comment lines delimit the rounds**: `<!-- loupe-earlier -->` opens the section, and an anchor precedes each round. loupe reads an anchored body back by these lines and by the generated tail (reconciliation marker, findings record when present, `loupe-meta`) alone, and only on lines outside a fence. It matches no footer and no chips row by its shape: it cuts the newest round by its anchor's counts. It refuses with `sticky` a malformed anchor, a `v=` other than `1`, a newest round whose `n` is not `K`, collapsed rounds numbered other than `K − 1`, `K − 2` and so on without a gap, a body with no anchor before `<!-- loupe-earlier -->`, an anchor inside the newest round, and text between `<!-- loupe-earlier -->` and the first collapsed round other than the heading and dropped-rounds line loupe writes for `K − 1` minus the rounds held. The dropped count is always computed, never read from that line. A body without anchors was read the same way by `<!-- loupe-round -->` before each round, and by the newest round's divider and footer. The allowlist refuses an HTML comment outside a fence in every authored field, so authored text can quote a delimiter but never move a round boundary. A body that has lost that structure, such as one reworded on GitHub, is refused with `sticky` rather than guessed at. That includes text added to the footer or before the first collapsed round, a newest collapsed round whose footer was removed, quoted or not, a findings record anywhere but the tail, which the next body would otherwise carry as a second record, a collapsed round that is not one `<details>` under loupe's `<summary>Round N · …</summary>` line, a round that is not exactly one `<details>` whose tags inside pair up and nest, or whose fence or HTML comment is left open, and collapsed rounds plus dropped ones that do not add up to `K − 1`, since the next edit would otherwise lose a round without saying so.
...
- **A collapsed round is carried as it is, even when a line of its quote lost its `>` on GitHub.** That line renders outside the quote. A finding's `</details>` line that lost it also closes the round's collapse early, so the rest of that round renders below it; loupe carries that too. In a body without anchors, only a hidden marker could tell that edit from an earlier layout's prose that ends in a quote and a line shaped like a footer, so loupe does not guess. There loupe knows a collapsed round's footer by its shape, so a newest round whose footer was removed on GitHub is still carried when it has no findings and its own prose ends in a line shaped like one; that round keeps no footer from then on. In an anchored body the anchor's checksum names such an edit.
