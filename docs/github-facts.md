...
- Observed 2026-09-26: an edit keeps trailing whitespace byte for byte. Review 5326555969 on `eriksaulnier/loupe-probe#1` was edited with a body whose lines ended in two spaces, in a tab, and in a space and a tab. The `body` read back through `GET /repos/{owner}/{repo}/pulls/{number}/reviews/{review_id}` equaled the body sent, final newline included. Review creation was not probed for this.
...
- Observed 2026-09-26 for `specs/033-round-anchors`: a body that opens on an HTML comment line renders nothing above the next block. `POST /markdown` in `gfm` mode rendered an anchor line, a blank line and a chips row as HTML opening on the chips paragraph, and did the same with no blank line. A `COMMENT` review posted on `eriksaulnier/loupe-probe#1` (review 5326555969), opening on a round anchor with an anchored collapsed round below, came back with `body_html` opening on the chips paragraph and `body_text` on the chips text. `gh pr view --comments` in a terminal showed the chips row first. Piped, it prints the raw Markdown, comment included, as it does for every marker. Email notifications were not checked.
