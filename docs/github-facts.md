...
- Observed 2026-09-21 on an issue comment on `eriksaulnier/loupe#21`, read back as GitHub's rendered HTML: `<code>` inside `<summary>` survives the sanitizer and renders as code. So does `<code>` in a Markdown paragraph, where backslash escapes between and inside the tags decode. The same bytes in a review body or an inline review comment were not tried.
