...
func TestCodesMatchContractTable(t *testing.T) {
	want := []string{"usage", "no-run", "record", "origin", "pr", "same-head", "auth", "input", "location", "markdown", "version", "count", "not-found", "lock", "tty", "head-moved", "own-pr", "blocking", "not-ready", "empty", "attempt", "github", "internal"}
	got := []Code{Usage, NoRun, Record, Origin, PR, SameHead, Auth, Input, Location, Markdown, Version, Count, NotFound, Lock, TTY, HeadMoved, OwnPR, Blocking, NotReady, Empty, Attempt, GitHub, Internal}
	if len(got) != len(want) {
