...
func NewRoot(deps Deps) *cobra.Command {
	...
	root.AddCommand(newCaptureCmd(deps), newAddCmd(deps), newEditCmd(deps), newSummaryCmd(deps), newShowCmd(deps), newFeedbackCmd(deps), newReplyCmd(deps), newReviewCmd(deps), newPublishCmd(deps))
	return root
}
