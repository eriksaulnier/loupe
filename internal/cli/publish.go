...
sorting. Esc leaves the message; then j/k, up/down, pgup/pgdown and home/end scroll, v switches
to the exact JSON payload, and Tab goes back to the message. Only y sends, and only from outside
the message, so a y you typed cannot publish. Any other key, Ctrl-C or end of input cancels and
nothing is sent or written. Once the review is being sent, keys, Ctrl-C and SIGTERM do not stop
loupe until the outcome is recorded.
