...
loupe review on the pull request, or unattended, the newest one a [bot] posted from the same
loupe capture --source, version aside; unattended --sticky refuses a run captured without one, and
an unattended edit GitHub refuses as not its own names --source as the fix. An edit sends no
...
defaults to comment, --inline stays at its default none, and any other value of either refuses
with usage: an edit cannot change a review's state, and an inline comment stays on the review that
created it. A sticky review loupe cannot read back, such as one reworded on GitHub, refuses with
sticky; publish without --sticky to post a new review. loupe review's own publish step never
edits a review.
