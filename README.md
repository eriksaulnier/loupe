...
pi install git:github.com/eriksaulnier/loupe@v0.11.0 # x-release-please-version
```
...
Inside [Herdr](https://herdr.dev) or [Orca](https://www.onorca.dev/), `loupe handoff` opens `loupe review` in a split beside the agent's pane. The pane closes when review exits cleanly. In Orca the pane never takes focus, so a background agent never pulls your view away: the pane waits in the agent's tab. Elsewhere, or when the split fails, the skill asks you to run `loupe review` yourself.
