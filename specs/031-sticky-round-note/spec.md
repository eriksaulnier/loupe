...
**Input**: Owner request, 2026-09-25. The shared Claude review workflow (`traackr-internal-engineering/github-actions-library`, `actions/loupe-publish/publish.sh`) appends a hint to each round's summary in request mode: "Pushed more commits? Add the `claude-review-requested` label for a fresh review of the whole PR." A collapsed round keeps its summary, so the hint repeats in every earlier round, as on `traackr-internal-engineering/claude-review-testbed#4`. The pipeline needs a place for text that belongs to the newest round only.
