...
Amended 2026-09-24 (spec FR-025, owner's decision): never switch. The FR-024 gate passed for a batch review agent, which is nearly always the only tab in its worktree, so `switch` pulled the human to that worktree from whatever they were watching. `orca worktree show --worktree active` looked like the missing signal but resolves the worktree of the current directory, checked from four directories, and no other call reports the viewed worktree. Two live splits into a background worktree's tab left the owner's view where it was. The cost is the one the rejected alternative named: a foreground handoff no longer takes focus, and the human clicks into the pane beside the agent.
