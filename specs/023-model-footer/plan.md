...
- **The demo.** Decision: seed realistic values. `cmd/loupe-demo/seed.go` seeds `Source: "code-review@1.4.0"` and `Model: "anthropic/claude-opus-5.5"` instead of `loupe-demo` and `demo/reviewer-1`, so the README images show what a real footer looks like (owner, 2026-09-24). `docs/assets/review.png` and `review-dark.png` were redrawn with `mise run review-screenshot` against `eriksaulnier/loupe-sandbox#1`.
