...
- Rechecked on every pull request by `install-action-checksums`: a fake `curl` first on `PATH` serves a stand-in archive and a `checksums.txt` that is correct, lacks the archive's entry, or carries a wrong digest. The correct one MUST install, as a control, and the other two MUST fail. `base` stays a fixed URL, so `action.yml` gains no knob that exists only for a test (Principle VI). Review round 3 suggested this.
