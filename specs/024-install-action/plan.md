...
- **Download failure.** Decision: `curl -fsSL --retry 3`, and on failure `::error::could not download <url>. If <tag> was released in the last few minutes, its archives may still be uploading. Retry shortly. Otherwise check that <tag> is a loupe release.` `--retry 3`, which curl applies only to timeouts and HTTP 408, 429 and 5xx, per its man page. Rationale: FR-007 and the spec's assumption. A transient error from github.com does not cost the consumer a job, and a 404 still fails at once, so a wrong version is not hidden. Review round 3 raised this.
...
- Rechecked on every pull request by `install-action-inputs`: the default and then a pinned older release in the bare form (`0.9.0`) in one job, where the later `PATH` entry MUST win and the first binary MUST stay in place. An explicit empty input and a tag with no release (`v0.0.0`) MUST each fail. The empty case relies on the runner applying a default only to an unset input, which the first CI run confirms. Review round 4 raised both.
- Rechecked on every pull request by `install-action-checksums`: a fake `curl` first on `PATH` serves a stand-in archive and a `checksums.txt` that is correct, lacks the archive's entry, or carries a wrong digest. The correct one MUST install, as a control, and the other two MUST fail. After the refusals in this job and in `install-action-inputs`, `PATH` MUST equal what it was before them. Each step has its own `GITHUB_PATH` file, so the inherited `PATH` is where a leak would show. Review round 6 raised this. `base` stays a fixed URL, so `action.yml` gains no knob that exists only for a test (Principle VI). Review round 3 suggested this.
