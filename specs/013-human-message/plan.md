...
`internal/publish/envelope.go:85` fills `render.Input.Summary` from `d.Summary` for every publication. It becomes conditional: the draft's summary when unattended, and otherwise a message the human types at the publish confirmation. `Preview` carries a composition closure so the confirmation renders the real body around the message and publication sends what that closure last produced, and `Options.Confirm` returns the message alongside the decision. The input is drawn into the previewed body at the slot the prose fills, so the human writes where their words will appear. Nothing about body composition, the draft on disk, the digest, reconciliation or the receipt changes. The published shape is identical; only the author of the opening prose is not.
...
- **The input is drawn into the body, at the slot the prose fills.** Decision: the confirmation asks `Compose` for a body built around a sentinel, splits the rendered body on it, and renders the textarea between the two halves; `Preview.Body` and the payload view keep coming from `Compose` with the real message. Rationale: FR-003 and the owner's reading of the first build, where a box below a scrolling preview left the human writing in one place and reading the result in another. The line breaks the input accepts follow from the same reading: prose that sits in the body is prose someone will want a paragraph break in. Alternative rejected: the notice slot at the foot of the screen, which is where the send-back note lives and is the cheaper layout, but it shows the words away from the review they open.

- **A block in the body has to announce itself.** Decision: the input carries a colored bar down every row, a background tint while it holds the keyboard, and a hint in place of its text while it is empty. Rationale: dropped into a page of rendered Markdown, an empty input is a blank line, and the owner read the first build as unclear. The bar is the one glyph that survives every tier (`▎`, `>` in ASCII), the tint is set on the textarea's own text style rather than wrapped around its rows, which carry resets that would end a background early, and the hint is the textarea's placeholder.

- **The findings stay readable while the message is written.** Decision: `pgup` and `pgdown` scroll the preview without leaving the input, and the view is pulled back to the input only by a keystroke that changed the message. Rationale: the message is about the findings, and a first build that pinned the input on screen made them unreadable exactly while the human needed them. The arrows belong to the text; these two are the keys a textarea has no use for.
...
- **Nothing on disk stores the message.** Decision: it lives in the review program's memory, on the model rather than the confirmation, so a cancel or a refusal after y leaves it there for the next confirmation in that session, and quitting ends it. There is no command that sets it and no file that holds it. Rationale: FR-007 and FR-008. A message on disk is a message an agent, an abandoned attempt or last round can pre-fill, and pre-filled text is approved unread — the exact failure Principle II exists to prevent. Memory inside one session cannot be written by any of them. Alternative rejected: storing it on the draft and clearing it at publication, which buys the same thing and reopens that hole for every path that writes a draft. Also rejected: discarding it on cancel, which was the first reading of FR-007 and which throws the opening away exactly when publication refuses for a moved head or a changed draft, having already made the human write it.
...
- **One line is enough for the plain fallback.** Decision: `ConfirmPlain` reads a single line before its existing `[y/N]` prompt; empty means no message, and a message that is not empty gets the body printed once more with it in place, so nobody answers y to a review they were not shown. Rationale: FR-011 and FR-003. The fallback exists for terminals that cannot run the full-screen interface at all, and building a multi-line editor out of a line reader would be building a worse version of the thing the other path already has.
...
internal/publish/envelope.go               # BuildInput carries the message and the id; the conditional at line 85
