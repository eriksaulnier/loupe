...
- **FR-010**: The confirmation MUST NOT lose its existing guarantee that a single keypress confirms and publication happens only after it. That keypress MUST be `y`, or `p` on the full-screen confirmation where there is a message input (amended by `specs/002-review-ux` §5.5). The plain prompt still takes `y` alone. A focused input MUST NOT let a keystroke meant for the message confirm the review.
