...
- [x] T010a [US1] Amend `specs/001-loupe-v1/data-model.md` (FR-013): the published set is what publish sends, `accepted` when attended and the whole publishable set under `--unattended`, with a dated pointer here; the digest row's claim that readiness makes the sets equal is limited to an attended publish. Add the same point to FR-026 in `spec.md`. Found in review: the plan had taken the old definition as settled.
