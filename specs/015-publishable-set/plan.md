...
**Scale/Scope**: One deleted function, three repointed call sites, four strings, one comment, one test assertion, seven lines of a contract, and the spec-001 amendments, including one definition in its data model.
...
- **The contract splits three ways.** Decision: `docs/comment-format.md` says `published` at the five lines describing what the review carries (`:45`, `:119`, `:216`, and the `--inline` table at `:232` and `:233`), `publishable` at the two describing the allowlist check at publish (`:256`, `:278`), and keeps ordinary English at `:158` and `:184`. Rationale: FR-005. `envelope.go` fills the body and the inline comments from one slice, and `:45` and `:236` promise that `--inline` never changes what the body contains, so the table and the body lines take one word. The allowlist check reads as a gate on the publishable set, and `:278` is the refusal it raises: under `--unattended` readiness is skipped, so a `pending` finding's body reaches that check and can refuse there. The two ordinary-English hits — a diff side "included inside the span" and "bidi overrides included" — are the word doing its everyday job. Note: T125's line numbers are stale, from before the file moved. They were regenerated from a fresh grep and most of the cited lines no longer hold the word.

- **The published set's definition predates spec 007.** Decision: amend `specs/001-loupe-v1/data-model.md:153` to "what publish sends: `accepted` when attended, the whole publishable set under `--unattended`", limit `:152`'s equality claim to an attended publish, and give FR-026 a line saying the same. Rationale: FR-013. The definition said `accepted` alone, and `specs/007-unattended-publish` FR-013 made that false: `envelope.go:65-67` builds an unattended body from `draft.PublishableSet`, and `internal/cli/publish.go` already tells the reader it "publishes the publishable set, accepted and pending alike". With the definition as it was, every `published` this specification writes would be false for an unattended review. Alternative rejected: `publishable` on every body line, which is true in both modes but leaves the definition wrong and makes "every publishable finding has exactly one home in the body" read as though pending findings reach an attended body. The review of this branch found the gap; the plan had taken the definition as settled.
...
specs/001-loupe-v1/data-model.md           # the published set's definition and the digest row's equality claim
