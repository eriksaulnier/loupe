...
2. **Given** the repository variable `LOUPE_AUTO_REVIEW` is anything but `true`, **When** the workflow is triggered, **Then** the job is skipped.
...
- **FR-028**: `.github/workflows/review.yml` MUST first be committed with a `workflow_dispatch` trigger taking a pull request number and no other trigger. Adding `pull_request` for types `opened` and `ready_for_review` MUST be a separate change, made only after the maintainer has judged a live run on a pull request they named.
- **FR-029**: The workflow's job MUST run only when the repository variable `LOUPE_AUTO_REVIEW` is `true`, and MUST skip draft pull requests, pull requests from forks, release-please pull requests and Dependabot pull requests.
- **FR-030**: Every job MUST build loupe from `main`, never from the pull request's head, into a path outside the checkout. The reviewing job MUST declare exactly `contents: read` and `pull-requests: read`; only the publishing job MAY declare `pull-requests: write`, and no agent MAY run in it. The reviewing action writes the token it is given into the workspace it hands to the agent, so a write token in that job is a write token the agent can read.
...
- **FR-033**: The agent step MUST run Claude Code through OpenRouter using the `OPENROUTER_API_KEY` secret, with the model taken from the repository variable `LOUPE_REVIEW_MODEL`. Its allowed commands MUST be exactly `loupe --help`, `--help` on any subcommand, `loupe add`, `loupe edit`, `loupe summary` and `loupe show`. Its prompt MUST point it at `loupe --help`, tell it to file findings, set a summary even with no findings, and not publish.
