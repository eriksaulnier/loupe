...
2. **Given** that review, **When** it is read on GitHub, **Then** the footer ends with ` · unattended`, after the source when there is one, and `loupe-meta` carries `unattended=1` directly after `round=`. The segment followed the round until `specs/009-review-footer`.
...
- **FR-016**: An unattended review's footer MUST insert ` · unattended` directly after `round N`, and its `loupe-meta` MUST carry `unattended=1` directly after `round=`. A review published without `--unattended` MUST be unchanged byte for byte. Amended on 2026-09-16 by `specs/009-review-footer`: ` · unattended` now ends the footer, after the source when there is one, since the footer no longer carries a round; `loupe-meta`'s `unattended=1` is unmoved. The byte-for-byte sentence compares an attended review with an unattended one at the same version — `--unattended` adds the segment and the marker key and nothing else — and does not freeze the attended footer against a later amendment.
...
- **FR-028**: `.github/workflows/review.yml` MUST first be committed with a `workflow_dispatch` trigger taking a pull request number and no other trigger. Any further trigger MUST be a separate change, made only after the maintainer has judged a live run on a pull request they named. The first such change adds `pull_request` for type `labeled`, acting only on the `ai-review` label: a round is asked for by labelling. The workflow MUST take that label off once a round has answered the request, whether it succeeded or failed, and MUST leave it on when no round ran, so that putting it back is how the next round is asked for; only `labeled` triggers the workflow, so removing it starts nothing. Rounds for one pull request MUST NOT run concurrently, since two publishing at once would post two reviews. A trigger that reviews a pull request nobody asked about, such as `opened`, remains a later decision. (Amended 2026-09-17: the staging this requirement describes is complete. `opened` and `ready_for_review` landed in `667e6b9` on 2026-09-16, alongside the move of the plumbing — the eligibility skips, the label removal, the concurrency group, capture, the agent step and the publishing job — into `eriksaulnier/loupe-workflows/.github/workflows/review.yml@v1`. `.github/workflows/review.yml` is now the caller, holding only what is loupe's own: the triggers, the `REVIEW_ENABLED` kill switch, the permission ceiling and the model. Every rule above still binds, now against that shared workflow rather than against a file in this repository. One reason has changed without its conclusion changing: removing the label still starts nothing, but because `unlabeled` is not among the `pull_request` types, not because `labeled` is the only one. A pull request MUST get at most one round per commit unless a further round is asked for by label or dispatch. The shared workflow meets that for automatic rounds through an already-reviewed gate, which skips when a `github-actions[bot]` review already carries that commit and this workflow's source; a label or a dispatch bypasses the gate, which is what preserves deliberate re-review. Two paths defeat the rule, and both belong to `eriksaulnier/loupe-workflows`. A pull request opened with the label already on fires both `opened` and `labeled`, and the automatic round's `unlabel` job does not run, so the label stays on to fire again. A label added or a dispatch started while an automatic round is running is a second round on the same commit that the gate will not stop either, and the concurrency rule above is all that stands between it and two published reviews; on `#13` on 2026-09-17 the two rounds' reviewing jobs overlapped instead, so that rule is not established. `specs/001-loupe-v1/validation.md` records what was observed.)
...
- **Example workflow**: `.github/workflows/review.yml`, this repository's own integration. It checks out the base, builds, captures, lays out the review directory, runs an agent, publishes unattended and uploads the data root. (Amended 2026-09-17: those steps are now `eriksaulnier/loupe-workflows/.github/workflows/review.yml@v1`, which checks out the default branch, installs a pinned loupe release rather than building one, then captures, lays out the review directory, runs an agent, publishes unattended and uploads the data root. `.github/workflows/review.yml` is the caller, and holds only the triggers, the kill switch, the permission ceiling and the model.)
