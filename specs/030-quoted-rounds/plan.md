...
- **Recognizing a quoted round.** Decision: a collapsed round is in the quoted layout when its content, the lines between its `<summary>` and its reconciliation marker, ends on `> ` and a footer line, or on a bare footer line directly under a line that starts with `>`. Rationale: every earlier layout ends that content on `---`, on a bare footer under a divider, or, for v0.11.0, on the round's own last line, so the footer is what tells the layouts apart. Keying on a leading `>` instead would take an older round whose prose opens on a blockquote for a quoted round and refuse it. The second shape is a quoted round whose footer lost its marker, which FR-006 refuses.
- **Refusals.** Decision: every collapsed round in the quoted layout MUST have `>` at the start of every non-blank content line, or read-back refuses with `sticky` naming the round. The newest collapsed round MUST keep its footer, and `keepsFooter` accepts a quoted footer. Rationale: FR-006. A line that lost its marker renders outside the quote, and carrying it would publish that again. A quoted footer removed on GitHub leaves no footer, so the rule that already refuses a newest round without one covers it. A line that starts with `>` and no space still renders inside the quote, so only the `>` is required.
