...
- A quoted round whose content is only blank lines panicked the first build of `quoteIntact`, since removed.
- `TestPillsAsWordsReachesAQuotedRound` pins T002 at the render level. It fails against the markdown package before T002.
- T009's mutations, run before the lost-marker refusal was removed: `keepsFooter` accepting any quoted last line fails the two newest-round footer-removed cases; ignoring a footer that lost its marker fails only `older round's footer lost its marker`; disabling the lost-marker check fails the five lost-marker cases; requiring `> ` rather than `>` fails the `>text` acceptance; detecting a quoted round by a leading `>` fails `TestReadStickyCarriesAnOlderRoundThatOpensOnAQuote` and two lost-marker cases.
- After a check on `eriksaulnier/loupe-sandbox#1` (a pending review, never submitted), the owner kept section headings at `###` inside the quote. T003's `> #### Must fix` now reads `> ### Must fix`. Rounds collapsed with `####` are carried as they are.
- The lost-marker refusal was removed after the CI review of #54: it refused a v0.11.0 round whose prose ends in a quote and a footer-shaped line. `TestReadStickyCarriesLegacyProseShapedLikeAQuotedFooter` failed before the removal, and `TestReadStickyCarriesAQuotedRoundThatLostAMarker` replaces the lost-marker refusal cases. `keepsFooter` also keeps a newest round's footer that lost its marker, so every line that loses one is carried.
- A second CI review of #54 found an empty round, quoted down to its footer, still refused when that footer lost its marker. `keepsFooter` now reads the round's last line, quoted or not and past a closing divider, as its footer. `TestReadStickyCarriesAnEmptyQuotedRoundThatLostItsMarker` failed before the change.
