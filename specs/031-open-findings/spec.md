...
Decided by the implementing agent under the owner's brief, pending owner review.
...
- Q: Does an assessment carry a note? → A: No. An attended round publishes its assessments under the human's name in a hidden record the human does not read line by line. A status word and copies of findings already published add no words the human did not see; a note would. The reason a finding is still open belongs in the summary, where a reader sees it.
