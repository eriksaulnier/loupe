...
- **When the timer runs.** Decision: each tick loads the draft and `handback.json`; if the version moved it applies the change; it schedules the next tick only while `draft.Awaiting` is non-empty. A send-back from this window, the refresh key, and any reload that finds something awaiting start it. Rationale: FR-005 as clarified. The last reply ends the chain in the same tick that shows it.
