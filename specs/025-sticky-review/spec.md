...
- **FR-025**: `loupe publish --unattended --sticky` on a run whose capture recorded no source MUST be refused with `usage`, fix `loupe capture <url> --source <name>`, before GitHub is contacted.
- **FR-026**: A definite 403 or 404 on an edit MUST refuse with `github` and a fix that names `--source` and publishing without `--sticky`, since only the review's author can edit it.
