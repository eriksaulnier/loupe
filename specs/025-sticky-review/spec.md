...
- **Earlier non-sticky loupe reviews**: a sticky round never edits a review without `sticky=` in its `loupe-meta`. It creates its own sticky review, and the earlier reviews stay as they are. A plain loupe review the same publisher posts after a sticky one ends that series, so the next sticky round starts a new sticky review (owner, 2026-09-25).
- **More than one sticky review by the same author**: the one with the highest review id is edited. The others stay as they are.
- **A human edited their sticky review on GitHub**: when the body still has the structure loupe reads back (FR-011), the edited text carries forward into the earlier rounds as it now reads. When it does not, publish refuses with `sticky` and names the review, and the fix is to publish once without `--sticky`. That plain review ends the series, so the next `--sticky` round creates a new sticky review instead of refusing again on every run.
...
- **Several publishers on one pull request** (owner, 2026-09-25): each publisher keeps its own sticky review. A person's round edits only the viewer's review, so two people never edit or read back each other's rounds. A person's login never ends `[bot]` and a pipeline's does, so a person and a pipeline never pick each other's review. Two Apps are told apart by the source their captures record (FR-006), since an installation token cannot read its own login. An unattended `--sticky` round with no source is refused (FR-025). Two Apps that record the same source name still collide: GitHub refuses the edit of the other App's review, and the `github` refusal names `--source` as the fix.
- **Round numbers with several publishers**: an attended `round=` counts the publications in the viewer's own data root, so it counts only their rounds. An unattended `round=` counts every `[bot]` loupe review on the pull request, from any App, as `specs/007-unattended-publish` FR-015 defines it, and this spec does not change that. A sticky review's collapsed `Round N` is its place in that review, so it is always the publisher's own. `loupe show --previous` reads the local data root, so it only ever reads the publisher's own rounds, and a pipeline's fresh root has none.
...
- **FR-006**: A sticky round MUST list the pull request's reviews and choose, among reviews that are not `PENDING`, whose author matches, and whose body carries a `loupe-meta` line outside any code fence with a `sticky=` key, the one with the highest id, but only when it is also the publisher's newest loupe review. When the publisher's newest loupe review carries no `sticky=`, a plain publish has ended the series, and the round MUST create a new sticky review. The author matches when it equals the viewer on the attended path, and when it ends `[bot]` on the unattended path. On the unattended path the review's `src=` name MUST also equal the name of the source the round's capture recorded, the part before any `@`, so a release that changes the version keeps editing the same review.
...
- **FR-027**: A calling workflow SHOULD serialize loupe's runs per pull request, for example with a GitHub Actions `concurrency` group keyed on the pull request, because two unattended edits of one review at once lose one round.
- **FR-011**: loupe MUST read the edited body back through lines outside code fences only, as the allowlist reads fences. It MUST locate the rounds by loupe-generated HTML comment lines, which the allowlist forbids in every authored field outside a fence, so authored text cannot move a round boundary. A body that lacks the expected structure MUST be refused with the new refusal code `sticky`. That includes one whose collapsed rounds plus its dropped count do not add up to `K − 1`, so no round is lost without a word, and one where a round it would carry forward, the shown round or a collapsed one, is not exactly one `<details>` whose tags inside pair up and nest, with no fence or HTML comment left open. The refusal MUST name the review URL, with the fix to publish without `--sticky`, which ends the series (FR-006) so the next sticky round does not refuse again.
