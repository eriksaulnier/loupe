...
- **Two unattended rounds publish at once**: each reads the same body and the later edit wins, so one round's content is lost from the history. loupe does not lock across machines. The workflow's concurrency group is the guard.
