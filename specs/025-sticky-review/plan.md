...
- **The body's tail is read by position.** Decision: a sticky body always ends with its reconciliation marker, findings record and `loupe-meta`, and the newest round ends `---`, blank, footer, after the earlier rounds before the 2026-09-25 amendment and before them since. The read-back checks that shape from the end and refuses `sticky` when it is not there. Rationale: the tail is entirely generated. Reading it by position needs no parser, and a hand-edited body that broke it is refused rather than guessed at.
- **What an earlier round holds.** Decision: its round part without the chips row, then its reconciliation marker, inside `<details>` with `<summary>Round N · reviewed <code>SHA</code> · CHIPS</summary>`. `N` is the round's place in the review, taken from `sticky=K` and counted down for older rounds. `SHA` is the hex code span of its footer. `CHIPS` is the chips row as text, or `no findings`. The chips row is recognized only when `loupe-meta` counts a finding and the chips add up to that count, so prose that looks like chips is never taken for them. Its `loupe-meta` is dropped. Its footer was dropped too until the 2026-09-25 amendment, and is now kept after a divider, before its reconciliation marker. Rationale: the owner read the live render on 2026-09-25. `round=` showed "Round 3" on a pull request with no visible rounds 1 or 2, and the footer repeated the commit the summary already named. A body from before this format reads back unchanged apart from renumbering. The kept reconciliation marker lets an attempt from that round still reconcile after a newer round edited over it (spec edge case). One `loupe-meta` per body keeps `strings.Contains(MetaPrefix)` round counting and every tool that reads the marker correct.
...
- **Finding the review.** Decision: `findSticky` in `internal/publish/sticky.go` keeps the reviews that are not `PENDING`, whose author matches the envelope's rule (`authorMatches`: the viewer attended, `[bot]` unattended) and whose `loupe-meta` carries a `sticky=` key anywhere on the line, and picks the highest id. Strict checking of the key stays in `ReadSticky`, so a tampered marker is refused rather than skipped for a second review. Rationale: FR-006. It reuses the author rule reconciliation already applies.
- **Telling two Apps apart.** Decision: an unattended round also requires the review's `src=` name to equal its own capture's source name, ignoring the `@version`, and refuses `--unattended --sticky` with no source (FR-025). Rationale: an installation token cannot read its own login, so every `[bot]` looks alike, and picking another App's review made GitHub refuse the edit on every run. The source is already recorded, already in `loupe-meta`, and already validated to one token. The version is dropped so a release does not start a new review. Alternatives rejected: learning the App's login from the first create's response, which a fresh data root never has; and falling back to create after a 403, which would post a second review for the same pipeline whenever a 403 had any other cause. With no source there is nothing to tell two Apps apart, so the round is refused rather than left to guess. Two Apps that choose the same source name still collide, and the 403 refusal names `--source` (FR-026).
...
- **Flag rules sit in two places.** Decision: `internal/cli/publish.go` defaults `--action` to `comment` under `--sticky` and refuses any `--action` or `--inline` other than `comment` and `none` with `usage` before resolving the run. `publish.Run` repeats the action and inline rule beside the send, as it does for `--unattended`. Rationale: FR-002 to FR-004. Since `specs/026-inline-default-none`, `none` is every publish's `--inline` default, so sticky mode needs no default of its own, only the refusal.
...
- **II. Nothing posts unread under a human's name.** PASS under 3.0.0. The one request edits a review the viewer published, and the author rule restricts it to one. The preview is the whole replacement body. The recheck after `y` refuses if the body being replaced changed. The new round's findings are accepted and its prose is typed at the confirmation, as today. Earlier rounds' findings and prose were accepted and typed in their own rounds, and they are shown again in full. Unattended keeps App identity, `COMMENT` only and the unattended marking, and edits only a `[bot]` review from its own source. Two Apps that record the same source name could still pick each other's review. The edit request is then expected to be refused by GitHub, which ends as a definite rejection that changes nothing. That refusal is a listed probe, not an observation.
