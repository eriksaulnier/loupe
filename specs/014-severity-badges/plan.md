...
- **Blocking is said once too.** Decision: the summary line drops ` (blocking)`. Rationale: FR-028. The contract already recorded the word as "redundant against the heading and kept anyway so the inline surface, which has no heading, keeps the signal" — but the inline surface leads with the `⛔` dot, so it was saying it twice on one line, and the two surfaces already differ by that dot. With the word gone, blocking is carried by the `⛔ N blocking` chip, the `### ⛔ Blocking` heading and the inline dot; the label word stays, because the dot says that a finding blocks, not what kind of remark it is. This amends the answer in `specs/001-loupe-v1/spec.md`'s clarification log.
...
- **Severity gives way last.** Decision: the two existing steps are untouched — the label column drops below 100 columns, the location shortens to a filename below 80 — and severity goes only when the title cannot reach its floor even after the location has given up its width. Rationale: FR-020. Severity is now the higher-value of the droppable columns, so it outranks the label; the title outranks all three, because a row the reader cannot identify is worth nothing.
- **The order is the review's, not severity's alone.** Decision: `draft.Ordered` sorts by section, then severity, then label group, then id, which is byte for byte the sequence `render.Body` emits; `internal/section` holds the section order and the label groups, and both packages read it. Rationale: FR-001 and FR-001a. The first cut sorted by severity alone, and the two surfaces then disagreed wherever section placement and severity conflicted — a nonblocking `critical` came first in the terminal and second in the review. The automated review round on the pull request caught it, and it was reproduced at the `internal/publish` seam before being fixed. The cost is that editing a label or a blocking flag now moves a row; the cursor already follows the finding rather than the position, so nothing is lost but the stillness.
...
- **What this change cannot check.** Decision: how GitHub renders `<b>major · issue:</b>` inside a `<summary>`, and whether the words in a column make reviewers pick alike, go on `specs/001-loupe-v1/validation.md`'s Unverified list. Rationale: FR-027 and Principle VII. The first needs a review on a real pull request; the second needs reviews landing over time, beside the entry `specs/012-severity-meaning` already put there.
...
internal/section/section.go       # new leaf: the body's section order and label groups
