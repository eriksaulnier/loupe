...
- **An unrated finding.** It sorts after every rated one, in finding-id order, and renders exactly as it does today: no summary-line prefix, no column text, no chip. Severity is reviewer-reported and MUST NOT be inferred or defaulted, so treating an absent severity as `trivial` would invent a value the reviewer withheld.
