...
- Text or a blank line above the top anchor was added on GitHub: the round on top is carried as found, with that text, and named as edited. A body with no anchor before `<!-- loupe-earlier -->`, or an anchor inside the round on top, is refused with `sticky`.
...
- **FR-007**: A round whose checksum does not match MUST be carried as found and not refused. A collapsed round MUST keep its block byte for byte, and its anchor MUST be written again with its other fields as found and a checksum over the block. The round on top MUST be collapsed into one `<details>` under its summary line with its whole text quoted, and nothing removed or rewritten but the quote markers. The top anchor is read from line 1 only: a round on top whose anchor is not line 1 is carried the same way, with the lines above its anchor quoted first.
