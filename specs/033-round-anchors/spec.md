...
- Whitespace-only lines above the top anchor are not content. Read-back drops them, and the round reads as unedited and collapses to the same bytes as without them.
- Any other text above the top anchor was added on GitHub and is content. The round on top is carried as found, with that text and the lines around the anchor kept, and named as edited.
- A body with no anchor before `<!-- loupe-earlier -->`, or an anchor inside the round on top, is refused with `sticky`.
...
- **FR-001**: Each round of a sticky body MUST be preceded by one anchor line `<!-- loupe-round v=1 n=N commit=SHA … sha256=HEX -->`. The top anchor MUST be line 1 of the body. On read-back, whitespace-only lines above it are not content: they MUST be dropped and MUST NOT make the round read as edited. Any other line above it is content, and the round on top MUST be carried as edited (FR-007). Each anchor MUST stand alone, outside any fence, followed by a blank line. A non-sticky body MUST NOT carry one.
...
- **FR-006**: Read-back MUST refuse with `sticky` on a malformed anchor, a `v` other than 1, an anchor on top whose `n` is not `K`, collapsed `n` values that are not `K − 1`, `K − 2` and so on without a gap, a mix of anchors and the legacy delimiter, no anchor before `<!-- loupe-earlier -->`, an anchor inside the round on top, text between `<!-- loupe-earlier -->` and the first collapsed round other than the section head loupe composes for the implied dropped count, or a carried round that is not one balanced disclosure.
- **FR-007**: A round whose checksum does not match MUST be carried as found and not refused. A collapsed round MUST keep its block byte for byte, and its anchor MUST be written again with its other fields as found and a checksum over the block. The round on top MUST be collapsed into one `<details>` under its summary line with its whole text quoted, and nothing removed or rewritten but the quote markers. A round on top with text above its anchor is carried the same way, with the lines above its anchor quoted first.
...
- **Probe, 2026-09-26: GitHub keeps trailing whitespace on an edit.** Review 5326555969 was edited with lines ending in two spaces, a tab, and a space and a tab, and its `body` read back equal to what was sent. So the checksum reads line endings as LF and trims blank lines at both ends, and leaves trailing spaces and tabs on other lines as they are. Review creation was not probed for this; loupe's own bodies carry trailing whitespace only where authored text does.
