...
- **GitHub client injection**: `github.Client` is an interface with `PullRequest(owner, repo, number)`, `PullRequestForBranch(owner, repo, branch)`, `Viewer()`, `ListReviews(...)`, `CreateReview(...)`. The production implementation builds a go-gh REST client with `api.ClientOptions{Host: "github.com", AuthToken from gh}`. The fake in `testutil/fakegh` is an `httptest.Server`, and tests build the real go-gh client against it with a `Transport` that rewrites the scheme and host, so request encoding is exercised end to end. `PullRequestForBranch` uses `GET /repos/{owner}/{repo}/pulls?state=open&head={owner}:{branch}`; the branch's owner is the clone's origin owner. Zero results refuses with `no-run`. More than one result is possible, since one head branch can have an open pull request against each of several bases; that also refuses with `no-run`, listing each pull request's number and base in `details` and naming `--run <ref>` as the fix.
