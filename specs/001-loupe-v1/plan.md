...
│   └── lock.go                    # flock with timeout, holder pid, live-holder refusal text
...
- **Approve-with-blocking gate**: `publish` computes the publishable set and refuses `--action approve` with code `blocking` when any is blocking, after the `own-pr` gate and before the readiness gate. The TUI's action picker greys out approve with the same reason.
