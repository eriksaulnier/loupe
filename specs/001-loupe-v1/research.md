...
- Layout, keys and glyph defaults are amended by `specs/002-review-ux/spec.md`; the bullets below follow it.
- Every full-screen view opens with one flat header, closed by a dim rule on every screen but the confirmation: the current object or step, the run when useful, and the readiness (`N pending · N open notes`, or `ready to publish`) at the right edge. Optional parts give way before the finding, position or readiness. Footers advertise arrow keys, only the actions that apply, and `? help`; help lists the Vim keys as aliases and scrolls when it is taller than the window.
- List: header with pull request, round and title, then the readiness counts (accepted, pending, excluded, withdrawn, open notes); the summary above the rows, collapsible; one row per finding with disposition glyph, a dim id, blocking marker, open-note marker (note color once the agent replied, dim before), title, label and `path:line`. The first pending finding is selected on open, then the first with an open note. Keys: `↑`/`↓` move (aliases `k`/`j`), `enter` opens, `tab` toggles the summary, `p` publish (always shown; dimmed as `publish (not ready)` until the draft is ready, while the header names what blocks it), `?` help, `q` quit. Quitting keeps every decision.
- Detail: one scrollable document: title and chips (disposition, blocking, `general` when unlocated, label, confidence, severity), then for a located finding the location and the hunk containing the anchor with anchored lines marked, then the body rendered by glamour, the suggested fix under a quote gutter, and notes with their replies. Keys: `←`/`→` previous and next finding (aliases `N`/`n`), `↑`/`↓`, `PgUp`/`PgDn` and space scroll the document (aliases `k`/`j`), `a` accept (included only; resolves the finding's open notes), `x` exclude (dismisses them), `s` send back (one-line note input), `u` restore an excluded finding to pending or reinstate a withdrawn one, which includes and accepts it (specs/011-reinstate-withdrawn), `e` edit label and blocking in a row under the finding (`←`/`→` label, space blocking, enter save, esc cancel; keeps the decision, specs/005-edit-in-review), `r`/`d` resolve or dismiss the open note without deciding, `f` file diff, `esc` back. Every decision carries the displayed draft version; a stale write is refused and the finding redisplays with a notice.
- File diff: the whole file's unified diff, scrollable, a one-cell gutter mark on every line with a finding and the id under the cursor in the header; `↑`/`↓` move (aliases `k`/`j`), `←`/`→` jump between findings (aliases `[`/`]`), `enter` opens the finding under the cursor, `esc` back. Added and removed coloring only; no syntax highlighting in v1.
- Publish from `p`: two unboxed steps, `Publish · Step 1 of 3` to pick the action (comment only on the viewer's own pull request; an unavailable action stays listed with its reason) and `Publish · Step 2 of 3` to pick the inline mode, then the confirmation view described under Publication. A refusal shows reason and fix and returns to the list.
...
- No lock is held while waiting for input. All rendering goes through one width-aware layer. A UTF-8 locale gets Unicode glyphs unless `LOUPE_ICONS` says otherwise; non-UTF-8 locales fall back to ASCII glyphs.
...
3. Refuse without a TTY on stdin and stdout (and stderr under `--json`, where the view draws), before reading the draft or credentials. Refetch the pull request. If the head differs from the captured head, compare the captured head to it (`GET /repos/{owner}/{repo}/compare/{captured}...{live}`, each side qualified as `{headOwner}:{headRepo}:{sha}` because a bare fork sha returns 404; see `docs/github-facts.md`): refuse with `head-moved` unless GitHub reports `ahead`, so a 404 or any other status means the captured commit left the pull request's history (fix: `loupe capture <url>`); refuse approve at a moved head with `head-moved` (fix: `--action comment` or `request-changes`, or `loupe capture <url>`); otherwise keep what the head gained for the confirmation, which shows the newest 20 commits since capture, a count of the earlier ones, and the included located findings on files they changed, and the review still goes out at the captured head (FR-043, clarified 2026-09-14). Refuse approve or request-changes when the viewer is the author. Refuse approve when any included finding is blocking (code `blocking`; fix: `--action comment` or `request-changes`, or exclude or unblock the finding in `loupe review`; clarified 2026-09-13). Refuse unless ready (fix: `loupe review`).
4. Build the envelope: target, viewer, action, commitId, draft version and digest, inline mode, a UUID publicationId, body and comments. The body follows `docs/comment-format.md`: count chips, summary, sections Blocking, Issues, Suggestions, Questions, Other with a `<details>` block per finding, `---` dividers, the footer `` loupe · round N · reviewed `sha` ``, where `N` is the review's position among the pull request's publications (FR-042, amended 2026-09-14), with `` · via `source` `` when capture recorded one, then the hidden `<!-- loupe digest=… publication=… -->` and `<!-- loupe-meta … -->` comments, the latter with `src=` when a source was recorded. Inline mode `none` (default, specs/026-inline-default-none), `blocking` or `all` selects which located findings also become review comments; general findings are never inline.
