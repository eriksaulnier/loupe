...
- **FR-004**: The note MUST stay one paragraph. `enter` MUST send it and `esc` MUST close the box without recording a note, as today. The top edge's way-out text MUST read `enter sends · esc cancels`, against the message box's `esc when done`, with the glyph tier's separator in place of `·` so the ASCII tier reads `enter sends - esc cancels`.
...
- **FR-006**: The box MUST occupy the notice slot where the note opens today. Closing it MUST give the slot back to the notice line: empty after `esc`, and the send's own notice or refusal after `enter`.
